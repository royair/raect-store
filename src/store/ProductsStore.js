import { makeAutoObservable, makeObservable, action, observable, computed } from 'mobx';
import faker from 'faker';
import _ from 'lodash';
import { v4 as uuidv4 } from 'uuid';
import numeral from 'numeral';

import rootStore from './RootStore';

export default class ProductsStore {
  @observable products = [];

  constructor(rootStore) {
    makeObservable(this);

    this.rootStore = rootStore;
    this.init();
  }

  @action init = () => {
    this.products = mockProducts.map((product) => new Product(product));
  };

  @computed get productsOrderedByETA() {
    return _.orderBy(this.products, 'eta');
  }

  @computed get unreceivedProducts() {
    return this.productsOrderedByETA.filter((product) => !product.isReceived);
  }

  @computed get receivedProducts() {
    return this.productsOrderedByETA.filter((product) => product.isReceived);
  }

  @computed get groupedProductsByStoreName() {
    return _.groupBy(this.productsOrderedByETA, 'storeName');
  }
}

class Product {
  id;
  name;
  storeName;
  _price;
  eta;
  imageUrl;
  isReceived;

  constructor(product) {
    const { id, name, storeName, price, eta, imageUrl } = product;

    this.id         = id;
    this.name       = name;
    this.storeName  = storeName;
    this._price     = price;
    this.eta        = eta;
    this.imageUrl   = imageUrl;
    this.isReceived = false;

    makeAutoObservable(this);
  }

  setReceived = (value) => this.isReceived = value;

  get price() {
    const currentRate = rootStore.ratesStore.getCurrentRate();

    return {
      symbol: currentRate.symbol,
      value: numeral(this._price * currentRate.value)
        .format('0,0.00')
    };
  }
}

/*generate some fake products*/

const storeNames   = ['Amazon', 'ebay', 'Aliexpress', 'Nike', 'Adidas', 'Apple'];
const mockProducts = [...Array(10)
  .keys()].map(() => ({
  id: uuidv4(),
  name: faker.commerce.productName(),
  storeName: _.sample(storeNames),
  price: Number(faker.commerce.price()),
  eta: faker.date.future()
    .toISOString(),
  imageUrl: 'data:image/png;base64,'
}));

//console.log(mockProducts);
